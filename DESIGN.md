# chowdy — быстрая face-аутентификация для Linux

> Читается как «C-howdy» — Howdy, только на C++. Дизайн-документ для замены оригинального Howdy на одной машине: резидентный C++ daemon, минимальный PAM-модуль, IR-only pipeline. Целевая среда — Arch Linux на ASUS ZenBook 13 Flip с IR-камерой `13d3:56eb` (IMC Networks).
...
└────────────┬─────────────┘
             ...
             ▼
┌──────────────────────────┐      unix socket      ┌──────────────────────────┐
│   pam_chowdy.so        │ ────────────────────▶ │       chowdyd          │
│   (tiny C, runs as root) │  /run/chowdy/auth   │   (C++, uid=chowdy)    │
│                          │ ◀──────────────────── │                          │
...
- `auth.sock` — для PAM, права `0660 root:chowdy`. Только аутентификация, минимальный API.
- `mgmt.sock` — для CLI, права `0666`. Read-only операции (list, test) доступны любому uid для собственных данных (`SO_PEERCRED`). **Мутирующие операции (enroll, remove) принимаются только от peer uid 0** — добавление лица = добавление учётного фактора, оно обязано пройти существующую аутентификацию (пароль sudo). Целевой uid передаётся в запросе (CLI берёт из `SUDO_UID`). Исходная формулировка «работает с энроллментами вызывающего uid» оказалась дырой: любой процесс залогиненного юзера (или человек у разлоченной сессии) мог заэнроллить чужое лицо и получать sudo по нему.
...
| Locally логиненный атакующий читает чужие эмбеддинги | `/var/lib/chowdy/users/<uid>/` mode 0700, owned by `chowdy:chowdy`. PAM-модуль и CLI ходят только через daemon. |
...
| Подсадка чужого лица через разлоченную сессию (malware под uid юзера или человек у незаблокированного ноута делает enroll своего лица → проходит sudo как юзер) | Мутирующие операции mgmt (enroll, remove) принимаются только от peer uid 0 — CLI обязан идти через sudo, т.е. атакующему нужен пароль. Read-only (list, test) остаются без эскалации. |
...
> chowdy — это удобство, а не более сильная аутентификация чем пароль. Принципиально слабее пароля или хардварного ключа. Не используйте как единственный фактор. PAM правило должно быть `sufficient`, не `required`.
...
Все в формате ONNX, кладутся в `/var/lib/chowdy/models/`:
...
Read-only запросы (`list`, `test`) CLI шлёт под uid пользователя — daemon отвечает данными этого uid. Мутирующие (`enroll_*`, `remove`) требуют запуска CLI через sudo: daemon проверяет `SO_PEERCRED == 0` и берёт целевого пользователя из поля `uid` запроса (CLI заполняет из `SUDO_UID`).
...
/var/lib/chowdy/
  models/                          # root:chowdy 0750
    detector.onnx
    embedder.onnx
    antispoof.onnx                 # опционально
  users/                           # chowdy:chowdy 0700
    1000/                          # chowdy:chowdy 0700, name = uid
      main.enc                     # chowdy:chowdy 0600
      glasses.enc

/run/chowdy/                     # tmpfiles.d создаёт, chowdy:chowdy 0755
  auth.sock                        # root:chowdy 0660
  mgmt.sock                        # chowdy:chowdy 0666

/usr/lib/security/
  pam_chowdy.so                  # root:root 0755

/usr/bin/
  chowdyd                        # root:root 0755
  chowdy-cli                     # root:root 0755
...
[recognition]
detector_model = "/var/lib/chowdy/models/detector.onnx"
embedder_model = "/var/lib/chowdy/models/embedder.onnx"
...
model = "/var/lib/chowdy/models/antispoof.onnx"
threshold = 0.7
...
|---|---|---|---|
| `chowdyd` | `chowdy` | none | `/dev/video2` (через group `video`), `/var/lib/chowdy/**` (own), `/run/chowdy/*.sock` (own) |
| `pam_chowdy.so` | root (в контексте PAM) | none | только connect на `/run/chowdy/auth.sock`. Не читает `/var/lib/chowdy/`, не открывает камеру. |
| `chowdy-cli` | пользователь | none | только connect на `/run/chowdy/mgmt.sock`. |
...
ProtectHome=true
ReadWritePaths=/var/lib/chowdy /run/chowdy
PrivateTmp=true
...
```bash
getent group chowdy >/dev/null || groupadd -r chowdy
getent passwd chowdy >/dev/null || useradd -r -g chowdy -G video -d /var/lib/chowdy -s /sbin/nologin chowdy
```
...
```ini
[Unit]
Description=chowdy authentication socket

[Socket]
ListenStream=/run/chowdy/auth.sock
SocketUser=root
SocketGroup=chowdy
SocketMode=0660
Accept=false

ListenStream=/run/chowdy/mgmt.sock
SocketUser=chowdy
SocketGroup=chowdy
SocketMode=0666
...
```ini
[Unit]
Description=chowdy face authentication daemon
Requires=chowdyd.socket
After=chowdyd.socket

[Service]
Type=notify
ExecStart=/usr/bin/chowdyd
WatchdogSec=30s
Restart=on-failure
...
**Tmpfiles** для `/run/chowdy/`:
```
d /run/chowdy 0755 chowdy chowdy -
```
...
```
# chowdy ВСЕГДА sufficient, НИКОГДА required
...
│
├── pam/
│   ├── pam_chowdy.c              # один файл, ~150 строк
...
└── packaging/
    └── arch/
        ├── PKGBUILD
        ├── chowdy.install         # pre/post hooks для создания пользователя
        └── README
```
...
- Standalone утилита `chowdy-capture-test` — открывает `/dev/video2`, захватывает N кадров, пишет в `/tmp/`.
...
- Standalone утилита `chowdy-detect-test` — берёт JPEG из файла, прогоняет детектор, потом embedder, печатает confidence и embedding vector.
...
- Утилита `chowdy-pipeline-test`: open camera → capture loop → detect → embed → если есть `~/.cache/chowdy/test_embedding.bin`, сравнить cosine sim, иначе сохранить как «энроллмент».
- **Цель:** доказательство концепции — за сколько мс реально определяется лицо.

### M4: IPC и daemon (2-3 дня)
- `chowdyd` запускается, слушает сокет, отвечает на `test` запрос (без логики пока).
- `chowdy-cli test` подключается, получает ответ.
...
- **Цель:** `chowdy-cli test` показывает что ваше лицо распознаётся.

### M6: PAM модуль (2-3 дня)
- `pam_chowdy.c`, минимальный.
...
- Примеры PAM конфигов в `/etc/chowdy/pam-examples/`.
...
- **PAM ошибка может залочить.** ВСЕГДА:
  ...
  - всегда `auth sufficient pam_unix.so try_first_pass` ПЕРЕД строкой с chowdy в новых конфигах в первое время.
- **Daemon под root для отладки** — соблазнительно, но не делайте. Сразу настройте `User=chowdy` в systemd.
- **Не коммитить test embeddings в репо.** В gitignore: `*.enc`, `*.bin`, `~/.cache/chowdy/`.
...
- [ ] Перед первым тестом нового `pam_chowdy.so` — он тестируется на `sudo`, а не на login.
- [ ] В PAM конфиге chowdy — `sufficient`, никогда `required`.
- [ ] Бэкап `/etc/pam.d/` сделан до начала работы (`sudo cp -r /etc/pam.d /etc/pam.d.bak`).
- [ ] Daemon запускается под uid `chowdy`, никогда не под root в проде.
...
**Следствие для camera_policy:** `lazy` съедает 77% бюджета `< 400 мс` на cold open и не помещается в цель Section 1, **но** укладывается в пороговое `< 1500 мс p99` и даёт лучший UX (лампочка камеры не горит). Изначально дефолт планировался `idle_keep_ms=10000`, но это обоснование («серия sudo не платит auth дважды») оказалось неверным — sudo кэширует креды и второй раз PAM не зовёт. Реальные сценарии для `idle_keep` это только retry после failed auth — а 2–3 секунд для этого достаточно. **Default переключён на `lazy`** (см. также `etc/chowdy/config.toml.example`). Хочешь warm-перформанс — `camera_policy = "warm"` явно.
...
Лицензионная заметка: тестовые embeddings хранятся в `~/.cache/chowdy/test_embedding.bin` (формат FA01, см. §8). Это просто бинарный дамп 8 × 512 float, читаемый только локально и привязан к `embedder_id` (hash файла модели) — при апдейте модели daemon должен fail-with-message и просить переэнроллиться.

### M5 findings (резидентный daemon с реальным auth handler, 2026-05-24)

Рефактор `tools/m3_pipeline_test.cpp` в `daemon/{camera,models,pipeline,enrollment_store}.cpp` + integration в `handlers/auth.cpp` через `Pipeline*`/`EnrollmentStore*` в `Context`. Daemon под текущим юзером, сокеты в `/tmp/chowdy-smoke/`, enrollment из M3 переименован в `users/1000/main.enc`.
...
- Конфиг из `/etc/chowdy/config.toml` — M8; пока всё через `--detector / --embedder / --device / --users-dir` CLI флаги.
...
```
$ mkdir -p /tmp/chowdy-smoke/users/$(id -u)
$ cp ~/.cache/chowdy/test_embedding.bin \
     /tmp/chowdy-smoke/users/$(id -u)/main.enc
$ ./build/daemon/chowdyd --foreground --log-level info \
    --auth-socket /tmp/chowdy-smoke/auth.sock \
    --mgmt-socket /tmp/chowdy-smoke/mgmt.sock \
    --users-dir /tmp/chowdy-smoke/users \
    --detector models/scrfd_500m_bnkps.onnx \
    --embedder models/w600k_mbf.onnx &
$ ./build/cli/chowdy-cli \
    --mgmt-socket /tmp/chowdy-smoke/mgmt.sock \
    --auth-socket /tmp/chowdy-smoke/auth.sock auth-test
```
